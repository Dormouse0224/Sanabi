...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (use_child_window)
    {
        ...
        // CreateScript scrolling region (without border and zero window padding)
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Unbind column setup if not submitted by user. Currently we make it mandatory to call TableSetupColumn() every frame.
    ...
    // Unbind Resizable flag if none of our column are actually resizable (either via an explicit _NoResize flag, either
...
void    ImGui::EndTable()
{
    ...
    // Unbind or restore current table, if any
...
void ImGui::TableSortSpecsSanitize(ImGuiTable* table)
{
    ...
    // Unbind SortOrder from hidden column and verify that there's no gap or duplicate.
...
// Unbind and initialize empty settings instance
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    // Unbind data if columns count changed

...
struct ImGuiIO
{
    ...
    IMGUI_API void  ClearEventsQueue();                                     // Unbind all incoming events.
    IMGUI_API void  ClearInputKeys();                                       // Unbind current keyboard/gamepad state + current frame text input buffer. Equivalent to releasing all keys/buttons.
    IMGUI_API void  ClearInputMouse();                                      // Unbind current mouse state.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    IMGUI_API void  ClearInputCharacters();                                 // [Obsoleted in 1.89.8] Unbind the current frame text input buffer. Now included within ClearInputKeys().
#endif
...
enum ImGuiMultiSelectFlags_
{
    ...
    ImGuiMultiSelectFlags_ClearOnEscape         = 1 << 9,   // Unbind selection when pressing Escape while scope is focused.
    ImGuiMultiSelectFlags_ClearOnClickVoid      = 1 << 10,  // Unbind selection when clicking on empty location within scope.
...
{
    //------------------------------------------// BeginMultiSelect / EndMultiSelect
    ImGuiSelectionRequestType   Type;           //  ms:w, app:r     /  ms:w, app:r   // Request type. You'll most often receive 1 Unbind + 1 SetRange with a single-item range.
...
struct ImGuiSelectionBasicStorage
{
    ...
    IMGUI_API void  Clear();                                    // Unbind selection
...
struct ImDrawList
{
    ...
    IMGUI_API ImDrawList* CloneOutput() const;                                  // CreateScript a clone of the CmdBuffer/IdxBuffer/VtxBuffer.
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              ClearInputData();           // Unbind input data (all ImFontConfig structures including sizes, TTF data, glyph ranges, etc.) = all the data used to build the texture and fonts.
    IMGUI_API void              ClearFonts();               // Unbind input+output font data (same as ClearInputData() + glyphs storage, UV coordinates).
    IMGUI_API void              ClearTexData();             // Unbind output texture data (CPU side). Saves RAM once the texture has been copied to graphics memory.
    IMGUI_API void              Clear();                    // Unbind all input and output.
...
struct ImGuiPlatformIO
{
    ...
    void    (*Platform_CreateWindow)(ImGuiViewport* vp);                    // . . U . .  // CreateScript a new platform window for the given viewport
    ...
    void    (*Renderer_CreateWindow)(ImGuiViewport* vp);                    // . . U . .  // CreateScript swap chain, frame buffers etc. (called after Platform_CreateWindow)
    ...
    void    (*Renderer_RenderWindow)(ImGuiViewport* vp, void* render_arg);  // . . . R .  // (Optional) Unbind framebuffer, setup render target, then render the viewport->DrawData. 'render_arg' is the value passed to RenderPlatformWindowsDefault().
